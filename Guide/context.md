# GRYDAI: PROJECT CONTEXT & DOMAIN KNOWLEDGE
...
* **Presentation Format:** A live, interactive demonstration where presenters manually trigger hardware faults (potentiometers simulating EV current surges and voltage sags, buttons simulating line breaks) to show the AI predicting failures on a 3D digital twin dashboard in real-time.
...
* **Frontend:** React 18, Vite, Tailwind CSS, Lucide React, HTML5 Canvas. Features an interactive **3D Isometric Hospital Digital Twin** with status auras, responsive zoom controls, and an **Analytics View** with an Adaptive 3-Sigma MSE Reconstruction chart.
* **Backend:** Python FastAPI, WebSockets (for 10 Hz live streaming), NumPy / Pandas (for time-series data windowing).
* **Machine Learning:** Scikit-Learn. Pipeline utilizes a lightweight `MLPRegressor` Autoencoder (`hidden_layer_sizes=(8, 3, 8)`) to establish baseline reconstruction MSE paired with dynamic standard deviation ($Z$-score) thresholds and multi-tier anti-flicker consensus filtering.
* **Hardware/Embedded:** ESP32 Development Board (38-pin), programmed in C++ via Arduino IDE. PySerial handles the USB bridge with bidirectional command support.

---

## 5. Hardware Inventory & Physical Setup
The project explicitly avoids using live AC mains voltage to comply with safety rules. The grid is safely simulated using 3.3V/5V DC components.
...
* **Sensors (The Simulators):**
  ...
  * 1x Photoresistor/LDR [Optional / Stretch Goal] (Simulating solar grid drop-offs; defaults safely to 100% if unwired).
...
* **Prototyping Materials:** 830-point breadboard, jumper wires, data-sync USB cable. Four 390Ω resistors (three for LEDs, one for button pull-down).
...
## 7. Live Pitch Choreography (3-Minute Script)
...
3. **[1:30 - 2:30] The Live Hardware Action (The WOW Factor):**
   * **Step 1:** Show the 3D Hospital Command Portal with stable 220V / 15A / 50.0Hz telemetry streaming at 10 Hz from **Transformer Node #TR-408**. AI Status is neon green.
   * **Step 2:** Turn the Current potentiometer dial (simulating simultaneous EV fast-charging).
   * **Step 3 (Under 300ms):** The live Reconstruction MSE curve shoots straight up, the 3D hospital model pulses amber warning (Status 1), and the physical hardware Yellow LED clicks ON.
   * **Step 4:** Tap the push button (instant line break) $\rightarrow$ emergency Neon Red lockdown (Status 2) in $< 10\text{ms}$.
4. **[2:30 - 3:00] Economic & Carbon Impact:**
   * Point to the Impact Metrics: "$48,500 saved in transformer replacement; 42 minutes of downtime avoided for critical surgical suites; 2.4 metric tons of $CO_2$ peaker emissions avoided."
...
## 9. Data Communication Contracts

### Contract 1: Hardware to Backend (JSON via Serial @ 10 Hz)
```json
{
...
### Contract 2: Backend to Frontend (JSON via WebSocket ws://localhost:8000/ws @ 10 Hz)
```json
{
  "timestamp": 1694451234,
  "raw": { "v_raw": 2048, "i_raw": 2048, "fault_btn": 0 },
  "metrics": {
    "voltage_sim": 220.5,
    "current_sim": 15.2,
    ...
    "energy_percent": 42,
    "stability": 98.2,
    "waveform": [65, 70, 75, 80, 85, 80, 75, 70, 65, 60, 55, 50, 45]
  },
  "ai_prediction": {
    "anomaly_score": -0.85,
    "grid_status": 0,
...
* `anomaly_score`: Normalized score between -1.00 and 1.00 for display on the OLED.